    ...
    for (const step of workflowStep) {
      ...
      if (isFirstStep) {
        ...
        if (CREDIT_LIMIT > OUTSTANDING) {
          ...
          } else if (grandTotal <= OUTSTANDING && orderType == 'regular') {
      ...
      if (stepStatus === 'Completed' && shouldAutoApprove) {
        try {
          ...
          console.log(`DeliveryNotes created with DN number: ${dnNumber} for auto-approved step ${step.step_number}`);
          // Create DeliveryNotesDetails for each item
          if (items && items.length > 0) {
            for (const item of items) {

              await DeliveryNotesDetails.create({
                delivery_notes_id: deliveryNote.id, // Use the ID of the created DeliveryNotes record
                order_id: orderId,
                dn_item_details: null,
                item_name: item.product_name || 'Unknown Item',
                item_id: item.id,
                month_code: item.monthCode || new Date().toISOString().slice(0, 7), // YYYY-MM format
                order_qty: item.quantity || 0,
                dispatch_qty: item.dispatchQuantity || item.quantity || 0, // Default to order quantity if not specified
                dp: item.dp || 0,
                discount: item.discount || 0,
                amount: item.amount || '0'
              }, { transaction });
            }
            console.log(`Created ${items.length} DeliveryNotesDetails records for DN: ${dnNumber}`);
          } else {
